...
_run_geogrid         = f'{bashPath}/run_geogrid.sh '
_link_ungrib_output  = f'{bashPath}/link_ungrib_output.sh {ungribPath}'
...
with dag:
    ...
    run_geogrid = BashOperator(
        task_id='Geogrid',
        bash_command=_run_geogrid,
        dag=dag
    )

    link_ungrib_output = BashOperator(
        task_id='Link_Ungrib_Output',
        bash_command=_link_ungrib_output,
        dag=dag
    )
...
link_namelist_wps >> [run_geogrid, link_ungrib_output] >> run_metgrid >> update_namelist_input >> link_namelist_input >> run_real >> run_wrf
